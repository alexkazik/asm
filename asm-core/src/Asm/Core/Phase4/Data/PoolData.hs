...
addBlockToStartPool _ _                                 = [printInternalError|addBlockToStartPool: try to add afterwards|]
...
addVariableToDataPool _ _                                = [printInternalError|addVariableToDataPool: try to add afterwards|]
...
addPoolToPoolPoolC _ _                                = [printInternalError|addPoolToPoolPoolC: try to add afterwards|]

getByteValVector :: PoolData c -> Vector (ByteVal (Expr4 c))
getByteValVector (PoolDataFinal bvp) = bvpBytes bvp
getByteValVector _                   = [printInternalError|getByteValVector: the data it not yet final|]
