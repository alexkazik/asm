...
compile2 :: Cpu c => (Stmt2Block c, CompilerState1 c, CompilerWriter1 c) -> Error CompilerError (CompilerReader2 c, Stmt3Block c, CompilerState2 c, CompilerWriter2 c)
compile2 (x1, s1, w1) = do
  (x2, s2, w2) <- runRWST (lookupNamesC x1 >>= registerTypesC) r2 (initialState2 s1 w1)
  return (r2, x2, s2, w2)
  where
