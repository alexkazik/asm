...
import           Asm.Cpu6809.OpCodes.Parser
...
mkOpCodes source aliases = do
  ...
  let
    namesOprFn' = M.fromList $ map (\(n, _, _, c, _, _) -> (n, c)) operatorList
  ...
  typeOpcodes <- [t|IntMap (IntMap (Map AddressMode (Word16, Bool, Maybe FunctionKey)))|]
  let
    addOp
      :: (String, AddressMode, Bool, Word16, [Int], Maybe FunctionKey)
      -> IntMap (IntMap (Map AddressMode (Word16, Bool, Maybe FunctionKey)))
      -> IntMap (IntMap (Map AddressMode (Word16, Bool, Maybe FunctionKey)))
    addOp (n, am, is16, c, cpus, addr) os =
      let
        op = IM.singleton (fromIntegral $ namesOprFn' M.! n) $ M.singleton am (c, is16, addr)
        addCpu cpu ops = IM.unionWith (IM.unionWith M.union) (IM.singleton cpu op) ops
      in
