...
checkJmpIndC loc [x@(KDData TDInt, E4ConstInt _ e)] = do
  when (e < 0 || e > 0xffff) $ printErrorC $ (loc, "address is out of the address range (0-0xffff)"):[sourcePos||]
  when ((e .&. 0x00ff) == 0x00ff) $ printErrorC $ (loc, "jmp (x); lo will be read from " ++ show e ++ " and high from " ++ show (e - 0xff)):[sourcePos||]
  return (FnrResult x)
...
checkLaxImmC loc [x@(KDData TDInt, E4ConstInt _ e)] = do
  when (e /= 0) $ printErrorC $ (loc, "lax is only allowed with immediate 0 (or other addressing modes)"):[sourcePos||]
  return (FnrResult x)
